...
if page == "Main Dashboard":
    ...
    with tab2:
        bins = [0, 30, 60, 90, 180, 365, filtered_df['averageleadtime'].max()]
        ...
        lead_cancel = filtered_df.groupby('lead_time_bin').agg({
            'bookingscanceled': 'sum',
            'total_bookings': 'sum'
        }).reset_index()

        lead_cancel['cancellation_rate'] = (
            lead_cancel['bookingscanceled'] / lead_cancel['total_bookings'] * 100
        ).round(1)
    ...
    with tab3:
        ...
        numeric_df = filtered_df[corr_cols].select_dtypes(include='number')
        corr = numeric_df.corr()
        revenue_corr = corr[['total_revenue']].drop(index='total_revenue')
        ...
        fig = px.imshow(
            ...
            title='Correlation with Total Revenue'
        )
